...
class DemoViewController: SimpleMenuViewController {
	...
	override func viewDidLoad() {
		...
		self.title = "Modules"
		self.targets = [
			("Alerts", AlertsViewController.self)
		]
		
		let logo = UIImageView(frame: CGRect(origin: .zero, size: CGSize(width: self.view.frame.width, height: 200)))
		logo.image = UIImage(named: "logo-grad")
		logo.contentMode = .scaleAspectFit
		
		
		self.table.tableHeaderView = logo
	}
	//
