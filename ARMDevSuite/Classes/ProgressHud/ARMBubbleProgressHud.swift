...
public struct ARMBubbleProgressHudDefaultConfiguration {
	...
	// Bubble Appearance
	public var colorPrimary = UIColor(hex: 0xEB592E)
	public var colorSecondary = UIColor(hex: 0xEA38A7)
...
public class ARMBubbleProgressHud: UIView {
	...
	public func showResult(success: Bool, title: String?, detail: String?) {
		...
		let targetColor = UIColor(rgba: zip(colors.first!.rgba, colors.last!.rgba).map(+).map({ (agg) -> CGFloat in
			return agg/2
		}))
	...
	private func formatBubbles() {
		...
		for i in 0..<numBubbles {
			...
			let thisColor = UIColor(rgba: zip(start, step).map(+))
	...
	private func positionContentView() {
		...
		titleLabel.frame.size = CGSize(width: self.frame.width, height: max(UILabel.getEstimatedLineHeight(for: titleFont), titleLabel.frame.height))
		detailLabel.sizeToFit()
		detailLabel.frame.size = CGSize(width: self.frame.width, height: max(UILabel.getEstimatedLineHeight(for: detailFont), detailLabel.frame.height))
